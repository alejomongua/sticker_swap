module DashboardState
  ...
  private
    def prepare_dashboard_state
      ...
      state = current_dashboard_state_params

      @duplicate_mode = duplicate_mode_param(state)
      @duplicate_prefix = state["duplicate_prefix"].to_s.strip.upcase.presence
      @duplicate_code = normalized_code_param(state["duplicate_code"])
      @missing_prefix = state["missing_prefix"].to_s.strip.upcase.presence
      @missing_code = normalized_code_param(state["missing_code"])
      @duplicate_filters = {
      ...
      @missing_page = missing_page_param(@missing_pages, state)
    ...
    def prepare_missing_table_state
      @duplicate_mode = duplicate_mode_param(current_dashboard_state_params)
      @missing_total_count = current_user.inventory_items.missing.count
    ...
    def current_dashboard_state_params
      @dashboard_state_params ||= params.to_unsafe_h.stringify_keys
    end

    def set_dashboard_state_params(raw_params)
      source = raw_params.respond_to?(:to_unsafe_h) ? raw_params.to_unsafe_h : raw_params.to_h
      @dashboard_state_params = source.stringify_keys
    end

    def duplicate_items_scope(state = current_dashboard_state_params)
      filtered_items_scope(current_user.duplicate_items, prefix: @duplicate_prefix, code: state["duplicate_code"])
    end

    def missing_items_scope(state = current_dashboard_state_params)
      filtered_items_scope(current_user.missing_items, prefix: @missing_prefix, code: state["missing_code"])
    end
    ...
    def duplicate_mode_param(state = current_dashboard_state_params)
      state["duplicate_mode"].to_s == "bulk" ? "bulk" : "single"
    end

    def duplicate_page_param(total_pages, state = current_dashboard_state_params)
      page = Integer(state["duplicates_page"], exception: false)
      page = 1 if page.nil? || page < 1
      [ page, total_pages ].min
    end

    def missing_page_param(total_pages, state = current_dashboard_state_params)
      page = Integer(state["missing_page"], exception: false)
      page = 1 if page.nil? || page < 1
      [ page, total_pages ].min
    end
